...
/// The timer used for cursor and text blinking (SGR 5/6).
/// Both blink in phase because they share this single timer.
blink_h: xev.Timer,
blink_c: xev.Completion = .{},
blink_c_cancel: xev.Completion = .{},

/// Shared blink phase. Flips every interval. cursor_blink_visible and
/// text_blink_visible track it so they are coherent by construction.
blink: bool = true,

/// When false, the next blink tick is skipped for the cursor only.
/// Set by .reset_cursor_blink to keep the cursor solid for one extra
/// tick after a keypress without disturbing the shared phase.
...
flags: packed struct {
    ...
    /// This is true when blinking text (SGR 5/6) should be visible.
    /// Toggled by the shared blink timer in sync with cursor_blink_visible.
...
pub const DerivedConfig = struct {
    ...
    pub fn init(config: *const configpkg.Config) DerivedConfig {
        return .{
            ...
            .blink_interval_ms = config.@"blink-interval".asMilliseconds(),
        };
    }
};
...
pub fn init(
    ...
    // Setup a shared timer for blinking the cursor and text (SGR 5/6).
...
fn threadMain_(self: *Thread) !void {
    ...
    // Start the shared blink timer for cursor and text (SGR 5/6).
    self.blink_h.run(
        &self.loop,
        &self.blink_c,
        ...
        switch (message) {
            ...
            .focus => |v| focus: {
                ...
                if (!v) {
                    ...
                    // If we're focused, immediately show cursor and restart the timer.
            ...
            .reset_cursor_blink => {
                // Make the cursor immediately visible. The buffer flag tells
                // the blink timer to skip the next cursor toggle, keeping the
                // cursor solid for one extra tick without disturbing the shared
                // phase (so cursor and text blink stay in sync).
                self.flags.cursor_blink_visible = true;
...
fn blinkTimerCallback(
    ...
    _ = r catch |err| switch (err) {
        error.Canceled => return .disarm,
            ...
            log.warn("error in blink timer callback err={}", .{err});
            unreachable;
        },
    };

    const t: *Thread = self_ orelse {
        log.warn("blink timer callback fired without data set", .{});
        return .disarm;
    };

    t.blink = !t.blink;

    // After a keypress (.reset_cursor_blink), cursor_blink_buffer is set to
    // false so we skip toggling the cursor for one tick, keeping it solid for
    // one extra interval without disturbing the shared phase.
...
/// Returns the blink interval in milliseconds. Both the cursor and text
/// blink timers use this. The value comes from the `blink-interval` config.
fn cursorBlinkInterval(self: *const Thread) u64 {
    if (std.valgrind.runningOnValgrind() > 0) {
        ...
        return self.config.blink_interval_ms * 5;
    }

    return self.config.blink_interval_ms;
}
